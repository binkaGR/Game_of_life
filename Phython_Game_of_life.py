...
#logging.basicConfig(format=format, level=logging.INFO,datefmt="%S")
for r in arr_1:
   for c in r:
      print(c,end = " ")
   print()
...
volue_ex=0
while pov<100000:
    tread1=threading.Thread(target=game_of_live, args=(arr_1, arr_2, row , 0, 30))
    tread3=threading.Thread(target=game_of_live, args=(arr_1, arr_2, row , 30, 60))
    tread1.start()
    tread3.start()
    #logging.info("tread1")
    ...
    #logging.info("tread2")
    #volue_ex=volue_ex+time.thread_time()
    #print(time.thread_time())
    #print(pov)
    pov+=1
